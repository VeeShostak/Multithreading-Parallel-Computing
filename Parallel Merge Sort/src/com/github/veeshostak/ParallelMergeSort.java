...
public class ParallelMergeSort {
	...
	public void parallelMergeSort(int low, int high, int numOfThreads) {
		...
		// if can't run in parallel, use regular merge sort. 
		// time slicing will be slower than regular merge sort
		if (numOfThreads <= 1) {
			mergeSort(low, high);
			return;
		}
		...
		// assign a separate thread for each subarray
		...
		// wait for thread to finish before merging it with this one, go into waiting state
		try {
			leftSorter.join();
			rightSorter.join();
